...
# Define allowed coordinate range
MIN_LAT, MAX_LAT = 12.77663, 13.366072  # Latitude range
MIN_LON, MAX_LON = 77.38693, 77.78642  # Longitude range
...
def book_taxi():
    ...
    #pickup_lat = st.number_input("Pickup Latitude", value=37.7749)
    #pickup_lon = st.number_input("Pickup Longitude", value=-122.4194)
    #dropoff_lat = st.number_input("Dropoff Latitude", value=37.8044)
    #dropoff_lon = st.number_input("Dropoff Longitude", value=-122.2712)

    # Geocode and validate the addresses
    pickup_valid = False
    dropoff_valid = False
    pickup_coords = None
    dropoff_coords = None
    ...
    if st.button("Book Taxi"):
        ...
        if user_id:
            ...
            if pickup_valid and dropoff_valid:
                ...
                st.error("Booking failed. Please enter valid locations within the allowed range.")
    ...
    if pickup_address and pickup_city and pickup_country:
        ...
        try:
            ...
            if pickup_location:
                ...
                if not (MIN_LAT <= pickup_coords[0] <= MAX_LAT and MIN_LON <= pickup_coords[1] <= MAX_LON):
    ...
    if dropoff_address and dropoff_city and dropoff_country:
        ...
        try:
            ...
            if dropoff_location:
                ...
                if not (MIN_LAT <= dropoff_coords[0] <= MAX_LAT and MIN_LON <= dropoff_coords[1] <= MAX_LON):
    ...
    # Display maps if coordinates are valid
    if pickup_valid:
        st.write("Pickup Location Map:")
        pickup_location_map = folium.Map(location=[pickup_lat, pickup_lon], zoom_start=13)
        folium.Marker(location=[pickup_lat, pickup_lon], tooltip="Pickup Location").add_to(pickup_location_map)
        st_folium(pickup_location_map, width=700, height=500)

    if dropoff_valid:
        st.write("Drop-off Location Map:")
        dropoff_location_map = folium.Map(location=[dropoff_lat, dropoff_lon], zoom_start=13)
        folium.Marker(location=[dropoff_lat, dropoff_lon], tooltip="Drop-off Location").add_to(dropoff_location_map)
        st_folium(dropoff_location_map, width=700, height=500)

    # Add a map at the bottom of the page centered on Bengaluru, India
    st.write("### Map of Bengaluru, India")
    bengaluru_coords = [12.9716, 77.5946]  # Latitude and Longitude of Bengaluru
    bengaluru_map = folium.Map(location=bengaluru_coords, zoom_start=12)

    # Add marker for Bengaluru
    folium.Marker(
        location=bengaluru_coords,
        popup="Bengaluru, India",
        tooltip="Bengaluru",
    ).add_to(bengaluru_map)

    # Display the map at the bottom of the page
    st_folium(bengaluru_map, width=800, height=500)

# END JF's OF ALTERATION
