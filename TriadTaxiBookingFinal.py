...
# Import necessary modules and libraries for the website & database
...
# Base64 encoding for using HTML with background image
...
def header(page_title):
    ...
    # Display the underline in title with a shorter width using HTML
...
def book_taxi():
    ...
    pickup_city = st.text_input("Pickup City (Must be Greensboro, High Point, or Winston-Salem)")
    ...
    dropoff_city = st.text_input("Drop-off City (Must be Greensboro, High Point, or Winston-Salem)")
    ...
    if st.button("Book Taxi"):
        ...
        if user_id:
            ...
            if dropoff_city not in allowed_cities:
                st.error(f"Drop-off City must be {', '.join(allowed_cities)}.")
                return
            ...
            if st.session_state.pickup_coords and st.session_state.dropoff_coords:
                ...
                # Assign random cab ID's
                cursor.execute("SELECT cab_id FROM Cab ORDER BY RAND() LIMIT 1")
                cab_id = cursor.fetchone()[0]

                # Insert trip details into the database
                trip_query = """
                    INSERT INTO Trip (userID, pickup_time, dropoff_time, booking_created, website_booking, mobile_booking) 
                    VALUES (%s, %s, %s, %s, %s, %s)
                """
                trip_values = (user_id, pickup_time, dropoff_time, datetime.now(), 1, 0)
                ...
                booking_query = "INSERT INTO Booking (booking_id, userID, cab_id, trip_id) VALUES (%s, %s, %s, %s)"
                booking_values = (booking_id, user_id, cab_id, trip_id)
...
def payments_page():
    ...
    <div style="
        ...
        font-size: 18px;
    ">
        🎉 Launch Promo: Pay What You Can! Minimum $10 per ride for the first month. 🚖
    </div>
    """
    ...
    payment_method = st.selectbox("Select Payment Method", ["Credit Card", "Debit Card", "Other"])
...
# Function to record payment into the database
...
# Main function to run the app & include navigation menu
def main():
